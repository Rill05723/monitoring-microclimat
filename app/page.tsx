  ...
  hum_air: {
    getStatus: (v: number) => {
      if (v < 50)
        return {
          status: "Kering",
          info: "Kelembapan udara berada di bawah rentang ideal.",
          action: "Tingkatkan kelembapan lingkungan.",
        };

      if (v <= 80)
        return {
          status: "Optimal",
          info: "Kelembapan udara sesuai kebutuhan tanaman.",
          action: "Tidak diperlukan tindakan.",
        };

      return {
        status: "Lembap",
        info: "Kelembapan udara terlalu tinggi.",
        action: "Tingkatkan ventilasi greenhouse.",
      };
    },
  },
  ...
  temp_soil: {
    getStatus: (v: number) => {
      if (v < 18)
        return {
          status: "Dingin",
          info: "Suhu tanah berada di bawah rentang ideal.",
          action:
      ...
      if (v <= 27)
        return {
          status: "Optimal",
          info: "Suhu tanah mendukung pertumbuhan tanaman.",
          action: "Tidak diperlukan tindakan.",
        };

      return {
        status: "Panas",
        info: "Suhu tanah berada di atas rentang ideal.",
        action:
  ...
  soil_pct: {
    getStatus: (v: number) => {
      if (v < 60)
        return {
          status: "Kering",
          info: "Kelembapan tanah berada di bawah rentang ideal.",
          action: "Segera lakukan penyiraman.",
        };

      if (v <= 80)
        return {
          status: "Optimal",
          info: "Kelembapan tanah berada pada kondisi ideal.",
          action: "Tidak diperlukan tindakan.",
        };

      return {
        status: "Basah",
        info: "Kelembapan tanah terlalu tinggi.",
        action:
...
export default function Dashboard() {
  ...
  const tempAir = rules.temp_air.getStatus(
    Number(latest.temp_air)
  );

  const humAir = rules.hum_air.getStatus(
    Number(latest.hum_air)
  );

  const tempSoil = rules.temp_soil.getStatus(
    Number(latest.temp_soil)
  );

  const soil = rules.soil_pct.getStatus(
